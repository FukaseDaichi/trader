...
- `jp-stock-ticker-curation`: Research Japanese stocks from up-to-date internet sources and update `tickers.yml` with source-backed selections. (file: `/Users/fukasedaichi/git/trader/skills/jp-stock-ticker-curation/SKILL.md`)

### How to use skills

- Discovery: Use the skill list above as the source of truth for this repository.
- Trigger rule: If the user mentions `jp-stock-ticker-curation` (with `$SkillName` or plain text), or asks to research JP stocks and update `tickers.yml`, load and follow that skill.
