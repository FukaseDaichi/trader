...
def run_retrain(output_path: Path, version: str) -> int:
    ...
    for ticker in TICKERS:
        ...
        try:
            ...
            if df is not None:
                warnings = list(
                    dict.fromkeys(
                        warnings + (df.attrs.get("validation_warnings", []) or [])
                    )
                )
            if df is None or len(df) < 60:
                entries.append(
                    _failure_entry(ticker, "insufficient_data", warnings=warnings)
                )
                continue
            ...
            result, info = train_ticker_bundle(
                featured, BACKTEST_GATE_CONFIG, label_cfg, model_cfg
            )
            if result is None:
                entries.append(
                    _failure_entry(
                        ticker, info.get("reason", "training_failed"), warnings=warnings
                    )
                )
                continue

            model_store.save_model_bundle(
                version, code, result["boosters"], result["metadata"]
            )

            # KPI gate for report continuity (uses the same label config).
            gate = evaluate_kpi_gate(
                featured, BACKTEST_GATE_CONFIG, label_config=label_cfg
            )
    ...
    if ok_entries:
        ...
        print(
            f"weekly: saved {len(ok_entries)} model bundles under version {version}; "
    ...
    if ok_entries and db.db_enabled():
        ...
        print(
            "weekly: DB disabled; model_registry registration skipped "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--version",
        default=None,
        help="Model version label (default per-ticker-v1-YYYYMMDD)",
    )
    return parser
