...
function ActionIcon({ action }: { action: Signal["action"] }) {
  ...
  switch (action) {
    ...
    case "MILD_BUY":
      return <ChevronUpCircle size={size} />;
    case "MILD_SELL":
      return <ChevronDownCircle size={size} />;
    case "SELL":
...
export default function SignalCard({ signal }: SignalCardProps) {
  const confidence = confidenceLabel(signal);
  const rawAction = signal.raw_action;
  const rawActionLabel =
    confidence === "自信なし" && rawAction && rawAction !== "HOLD"
      ? actionLabel(rawAction)
      : null;
  const displayReason =
    signal.reason && signal.reason.includes("KPI")
      ? "自信なしのため見送り（過去検証で基準未達）"
      : (signal.reason || signal.confidence_reason || "");
  ...
  return (
    <div className={clsx("p-6 rounded-xl border shadow-lg transition-all", actionCardClass(signal.action))}>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          <p className="text-xs text-slate-400 uppercase">判定の自信度</p>
          <div className={clsx("inline-flex mt-1 px-3 py-1 rounded-full text-sm font-bold border", confidenceBadgeClass(signal))}>
            {confidence}
          </div>
        </div>
      ...
      {rawActionLabel && (
        <div className="mt-4 rounded bg-slate-900/50 p-3 text-sm text-slate-300">
          予測上のアクション: {rawActionLabel}（自信なしのため見送り）
        </div>
      )}
