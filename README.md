...
- **コスト 0 円**: GitHub Actions (Free tier) + Stooq / yfinance (Free data)
- **データソース自動切替**: Stooq のデータが古い場合は yfinance にフォールバック
...
| データ取得 | Stooq + yfinance (自動フォールバック) |
...
├── src/
│   ├── config.py            # 設定読み込み
│   ├── data_loader.py       # Stooq 取得 + stale時 yfinance フォールバック
...
|---|---|---:|---|
| `TRADER_YF_FALLBACK_ENABLED` | Stooq が古い/失敗時に yfinance フォールバックするか | `true` | 通常は `true` 推奨。検証で Stooq 固定にしたい場合のみ `false`。 |
| `TRADER_DATA_STALE_OPEN_DAYS` | 「古いデータ」と見なす許容営業日遅れ | `0` | `0` は1営業日でも遅れたら切替。許容するなら `1` 以上へ。 |
| `TRADER_DATA_HTTP_TIMEOUT_SEC` | データ取得 HTTP タイムアウト秒 | `20` | 回線が不安定なら `30` などに延長。 |
