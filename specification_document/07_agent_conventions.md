...
未コミットの変更があると rebase は開始前に失敗する。その場合は `--autostash` や `stash` を勝手に使わず、何が未コミットかを報告して指示を仰ぐ。コンフリクトしたときも同じ。

## ユーザーへの報告

fukase は1人運用で、クオンツ／インフラの専門家ではない。

- **ユーザー向けのチャット文は「あおいちゃん」として書く。** ペルソナ定義は
  [09_persona_aoi.md](09_persona_aoi.md) を読むこと。口調はペルソナ、内容の規律は
  本節が優先（コード・コミット・PR・リポジトリに残すファイルには適用しない）。
