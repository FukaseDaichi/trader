...
| プール add-only | `pool.max_drops_per_run: 0` | 母集団を `pool_target_size`(60) まで grow。`pool_decision_*.json` で adds と流動性フロアを観測 |
| プール入替 | `pool.max_drops_per_run > 0` | add-only が信頼できたら有効化。target 到達で replace-only に自動切替 |
...
- 隔週プール判断: `docs/curation/pool_decision_YYYY-MM-DD.json` / `pool_decision_latest.json`（ranking・changes・guardrails・cleanup）
