...
このディレクトリの仕様は、ソースコードを正として整理した現行仕様です。2026-07-19 時点でリポジトリに存在する `main.py`、`src/`、`scripts/`、`web/`、`.github/workflows/`、`.claude/skills/`、`migrations/`、設定ファイルの実装に合わせています（Phase 0〜3 実装済み）。
...
| レイヤ | 役割 | 主な実装 |
|---|---|---|
| 日次シグナル | OHLCV取得 → 特徴量 → KPIゲート → 予測 → 5段階シグナル + ATR出口プラン → LINEダイジェスト | `main.py`, `src/data_loader.py`, `src/model.py`, `src/backtest.py`, `src/predictor.py`, `src/digest.py`, `src/notifier.py` |
...
| Phase 3 UX・堅牢化 | 学習バリアと整合したATR利確/損切/時間出口、TOPIXベンチマーク決済、実績ダッシュボード（`/performance`）、日次ダイジェスト・週次サマリ通知（リトライ付き）、active配線 | `src/predictor.py`, `src/digest.py`, `src/performance.py`, `src/notifier.py`, `scripts/weekly_performance_notify.py`, `web/src/app/performance/` |
...
2. 06:00 — preopen core:
   ...
   2. `main.py`: 銘柄ごとにデータ更新 → 特徴量（34テクニカル+11マクロ）→ KPI ゲート → 保存済みモデル推論（無ければ legacy 学習）→ 5段階シグナル。ゲート通過ロングには学習バリアと同じATR出口プランを付ける
