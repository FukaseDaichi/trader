...
始値に対して以下を検査する。いずれかに該当した場合、始値列のみを破棄し、
理由を標準出力にログする。

1. 有限かつ正であること
2. 前日比の絶対値が`_MAX_MARKET_DAILY_MOVE`を超えないこと
3. 同一日の`close / open - 1`の絶対値が`_MAX_MARKET_DAILY_MOVE`を超えないこと
...
| 始値の取得失敗 | 始値のみ破棄。終値とマクロ特徴量は無傷。現状に縮退 |
| 始値系列の不連続 | 同上。理由をログ |
| 始値と終値の基準ズレ | 同上。理由をログ |
| 一部日付のみ欠損 | `_benchmark_coverage()`が`coverage_ratio < 1.0`と`incomplete_same_basis_coverage`を報告し、gateは不合格のまま |
